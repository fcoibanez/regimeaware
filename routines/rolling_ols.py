...
import random
import argparse
from regimeaware.constants import DataConstants, SimulationParameters, ROLLING_WINDOW
...
if __name__ == "__main__":
    ...
    for phi in SimulationParameters.RISK_AVERSION.value:
        for i in iterations:
            fname = f"{DataConstants.WDIR.value}/results/rolling_ols/phi{phi}_iter{i}.pkl"
            ...
            for t in tqdm(
                ...
                Xt = fctr_rt[i].loc[: SimulationParameters.IS_PERIODS.value + t].iloc[-ROLLING_WINDOW:]
                Yt = sec_rt[i].loc[: SimulationParameters.IS_PERIODS.value + t].iloc[-ROLLING_WINDOW:]
